...
all_returns_df_droped=all_returns_df.copy()
train_returns_df = all_returns_df_droped.iloc[ :split_point]
test_returns_df = all_returns_df_droped.iloc[split_point:]

train_returns_df

risk_return_profile_train=topquant_ksk.get_RiskReturnProfile(train_returns_df,cash_return_daily_BenchmarkFrequency.loc[train_returns_df.index],BM.loc[train_returns_df.index])
risk_return_profile_test=topquant_ksk.get_RiskReturnProfile(test_returns_df,cash_return_daily_BenchmarkFrequency.loc[test_returns_df.index],BM.loc[test_returns_df.index])

# 최적 파라미터 찾기 (Benchmark 제외)
risk_return_profile_train_no_benchmark = risk_return_profile_train[:-1]  # Benchmark 행 제외

best_param = risk_return_profile_train_no_benchmark['Information_Ratio'].astype(float).idxmax()
...
best_param_idx = risk_return_profile_train_no_benchmark.index.get_loc(best_param)
print(risk_return_profile_train_no_benchmark.iloc[[best_param_idx,-1]].T)
print(risk_return_profile_test.iloc[[best_param_idx,-1]].T)
